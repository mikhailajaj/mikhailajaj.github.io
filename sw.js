...
async function handlePageRequest(request) {
  ...
  // For development: skip cache for dynamic routes (e.g., /projects/[slug])
  const isDynamicRoute = /\/projects\/[^\/]+\/?$/.test(url.pathname) ||
                         /\/blog\/[^\/]+\/?$/.test(url.pathname);
  
  try {
    const networkResponse = await fetch(request, { 
      redirect: 'follow',
      cache: isDynamicRoute ? 'no-store' : 'default'
    });
    
    if (networkResponse.ok) {
      // Only cache non-dynamic routes or cache with shorter TTL
      if (!isDynamicRoute || networkResponse.status === 200) {
        cache.put(request, networkResponse.clone());
      }
      return networkResponse;
    }
  ...
  // Only serve from cache if not a dynamic route in development
  if (!isDynamicRoute || url.hostname !== 'localhost') {
    const cachedResponse = await cache.match(request);
    if (cachedResponse) {
      return cachedResponse;
    }
