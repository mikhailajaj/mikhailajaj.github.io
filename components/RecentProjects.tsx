...
import { FaLocationArrow, FaGithub, FaCheckCircle, FaExternalLinkAlt } from 'react-icons/fa'
...
const ProjectCard = ({ project }) => {
  const { id, title, des, img, iconLists, link, metrics, github, categories = [] } = project;
  
  // Find corresponding enhanced project for case study link
  const enhancedProject = enhancedProjects.find(ep => ep.id === id.toString() || ep.title === title);
  ...
  return (
    ...
    >
      <PinContainer title={title} href={link}>
        ...
        <p className={`${accentColor} text-sm font-semibold mt-2 flex items-center`}>
        ...
        <div className='flex items-center justify-between mt-7 mb-3'>
          ...
          <div className="flex gap-3">
            {enhancedProject && (
              ...
              >
                <p className="flex lg:text-sm md:text-xs text-xs">
                  Case Study
                </p>
...
const RecentProjects = () => {
  ...
  return (
    <div className='py-20 px-4' id='projects'>
      ...
      >
        <Link href="/projects" className="inline-flex items-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
          View All Projects
